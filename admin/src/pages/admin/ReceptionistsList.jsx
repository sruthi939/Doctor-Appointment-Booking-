...
import { UserCheck, Plus, Mail, Lock, User, Phone, Clock } from 'lucide-react'

const ReceptionistsList = () => {
    const { receptionists, aToken, getAllReceptionists, backendUrl } = useContext(AdminContext)
    ...
    const handleAddReceptionist = async (e) => {
        e.preventDefault()
        try {
            const { data } = await axios.post(backendUrl + '/api/admin/add-receptionist', {
                name, email, password, phone, shift
            }, { headers: { aToken } })
    ...
    return (
        <div className='m-5 w-full max-w-6xl'>
            <div className='flex items-center justify-between mb-4'>
                <div>
                    <h1 className='text-lg font-semibold text-slate-800'>Receptionists List</h1>
                    <p className='text-xs text-slate-500'>Manage front-desk staff and check-in personnel</p>
                </div>
                <button 
                        ...
                        receptionists.map((item, index) => (
                            <div key={item._id || index} className='flex flex-wrap sm:grid sm:grid-cols-[0.5fr_2fr_2fr_1.5fr_1.5fr] items-center px-6 py-4 hover:bg-slate-50/80 transition-all text-slate-600'>
                                ...
                                <div className='flex items-center gap-3'>
                                    <div className='w-9 h-9 rounded-full bg-indigo-50 text-[#5F6FFF] flex items-center justify-center font-bold border border-indigo-200'>
                                ...
                                <p className='text-xs font-semibold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100 inline-block w-fit'>
            ...
            {showModal && (
                <div className='fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50'>
                    <div className='bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 w-full max-w-md shadow-xl text-slate-700'>
                        <h2 className='text-lg font-bold text-slate-800 mb-4'>Add New Receptionist</h2>
                        <form onSubmit={handleAddReceptionist} className='space-y-4'>
                            <div>
                                <label className='block text-xs font-semibold text-slate-600 mb-1'>Name</label>
                                <div className='relative'>
                                    <User className='absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400' />
                                    <input onChange={(e) => setName(e.target.value)} value={name} className='w-full border border-slate-200 rounded-xl pl-10 pr-3 py-2 text-sm outline-[#5F6FFF]' type="text" placeholder="Receptionist Name" required />
                                </div>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div className='relative'>
                                    ...
                                    <select onChange={(e) => setShift(e.target.value)} value={shift} className='w-full border border-slate-200 rounded-xl pl-10 pr-3 py-2 text-sm outline-[#5F6FFF] bg-white'>
                            ...
                            <div className='flex justify-end gap-3 pt-4 border-t border-slate-100'>
                                ...
                                <button type="submit" className='px-5 py-2 bg-[#5F6FFF] hover:bg-indigo-600 text-white text-xs rounded-xl font-medium cursor-pointer shadow-sm'>Save Receptionist</button>
                            </div>
                        </form>
                    </div>
                </div>
